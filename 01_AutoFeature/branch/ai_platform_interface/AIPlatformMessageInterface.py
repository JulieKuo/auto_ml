...
class AIPlatformMessageInterface(MessageInterface):
    def __init__(self, job_id, user_id, group_id, project_id, database_config={}, redis_config={}, user=None):
        ...
        if user is not None:
            if user == 1:
                ...
                self._redis_args['password'] = '1qaz@WSX3edc'
        ...
        def wrapper(self, *args, **kwargs):
            ...
            while True:
                try:
                    ...
                    count += 1
                    print(f"Redis connection error, trying reconnect... attempts: {count}")
                    if count > 10:
                        self.handle_error(e)
                        raise ConnectionError(f"3401@")
                        # raise redis.exceptions.ConnectionError(e)
            return value
        return wrapper
    ...
    def handle_error(self, e, error_code=None, ignore_most_exceptions=False):
        ...
        if isinstance(e, requests.exceptions.RequestException):
            # connection error
            sql = "UPDATE a_application_call_exe_log SET status = %s, modify_time = %s WHERE job_id = %s AND group_id = %s AND project_id = %s"
            val = (STATUS_CONNECTION_ERROR, self.get_create_time(), self._job_id, self._group_id, self._project_id)
            self._cursor.execute(sql, val)
            self._connection.commit()
            ...
            error_msg = f"{error_msg} [{self.get_current_time()}]"
            ...
            val = (STATUS_OTHER_ERROR, error_msg, self.get_create_time(), self._job_id, self._group_id, self._project_id)
            self._cursor.execute(sql, val)
            self._connection.commit()
    ...
    def get_create_time(self):
        self.check_connection()
        sql = "SELECT create_time FROM a_application_call_exe_log WHERE job_id = %s"
        val = (self._job_id,)
        self._cursor.execute(sql, val)
        create_time = self._cursor.fetchone()[0]
        self._connection.commit()
        return create_time

...
class MiceImputer(object):
    ...
    def __init__(self, missing_values="NaN", strategy="mean", axis=0, verbose=0, copy=True):
        ...
        self.copy = copy
        self.imp = SimpleImputer(missing_values=self.missing_values, strategy=self.strategy, axis=self.axis,
    ...
    def _process(self, X, column, model_class):
        ...
        predict_indices = np.where(mask is False)[0]
        np.put(X, predict_indices, np.take(y, predict_indices))
